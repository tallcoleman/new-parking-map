...
InputProps = TypedDict("InputProps", {
  ...
  'MUNICIPALITY': Required[
    Literal['Etobicoke', 'North York', 'Scarborough', 
            'York', 'East York', 'former Toronto']
  ],
...
def _convert_ward_name(input_val):
  match = re.search(
    r"\s*?(?P<ward_name>.+?)\s*?\(\s*?(?P<ward_number>\d+?)\s*?\)",
    input_val
    )
  if match:
    return {
  ...
  return (
    {
      ...
      "capacity": _override_capacity(
        input_props['BIKE_CAPACITY'], 
        input_props['ADDRESS_FULL']
      ),
      ...
      "name": _get_place_name(
        input_props['PLACE_NAME'],
        input_props['TRANSIT_STATION']
      ),
      ...
      "meta_has_tools_and_pump": input_props['TOOLS_PUMP'], #  amenity=bicycle_repair_station
      "meta_has_vending_machine": input_props['VENDING_MACHINE'] # amenity=vending_machine
    } 
