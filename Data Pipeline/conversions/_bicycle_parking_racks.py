...
InputProps = TypedDict("InputProps", {
  ...
  'SURFACE': str, 
  'STATUS': Required[str],        # 'Delivered', 'Installed', 'Approved', 'Proposed', 'TBD'
  'LOCATION': Required[str], 
  'NOTES': Required[str],
  'MAP_CLASS': Required[
    Literal['Multi-Bike Rack', 'Multi-Bike Rack (Angled)', 
            'Bike Shelter', 'Bike Corral']
  ]
}, total=False)
  ...
  return (
    {
      ...
      "description": "\n\n".join(x.strip() for x in [
        ...
        _convert_seasonal(input_props['SEASONAL']).get('meta_about_seasonality', "")
      ] if x.strip()),
