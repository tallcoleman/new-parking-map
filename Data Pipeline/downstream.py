...
def group_proximate_rings(rings, radius=5.0):
  ...
  rings_UTM17N = rings_UTM17N.drop('geometry', axis=1).rename(columns={'cluster_centroid': 'geometry'})
...
def group_proximate_racks(racks, radius=30.0):
  ...
  sources_per_cluster = dict(racks_UTM17N_clusters[['cluster', 'source']].groupby('cluster')['source'].unique().apply(lambda r: len(r)))
  sources_per_cluster_test = racks_UTM17N_clusters['cluster'].apply(lambda c: sources_per_cluster[c] > 1)
  ...
  # dissolve clusters
  racks_UTM17N_clusters = racks_UTM17N_clusters.dissolve(by="cluster", aggfunc=aggregations)

  # get centroid and set as geometry
  racks_UTM17N_clusters['geometry'] = racks_UTM17N_clusters.centroid

  # combine racks
  racks_UTM17N_recombined = pd.concat([racks_UTM17N_clusters, racks_UTM17N_singles]).drop('cluster', axis=1)
